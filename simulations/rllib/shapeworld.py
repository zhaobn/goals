...
@dataclass(frozen=True)
class Action:
    feature: Literal['sides', 'shade', 'texture']
    actor: Literal[1, 2, 3]
    ...
    def __post_init__(self):
        ...
        if self.feature not in ['sides', 'shade', 'texture']:
            raise ValueError("Invalid feature")
        if self.actor not in [1, 2, 3]:
            raise ValueError("Invalid actor")
        if self.recipient not in [1, 2, 3]:
...
class ShapeWorld(MarkovDecisionProcess[State, Action]):
    ...
    def __init__(self, goal: State, discount_rate: float):
        ...
        # Set up action space
        self.action_space = [
            Action(feature='sides', actor=1, recipient=2),  # a1r2
            Action(feature='sides', actor=1, recipient=3),  # a1r3
            Action(feature='sides', actor=2, recipient=1),  # a2r1
            Action(feature='sides', actor=2, recipient=3),  # a2r3
            Action(feature='sides', actor=3, recipient=1),  # a3r1
            Action(feature='sides', actor=3, recipient=2),  # a3r2
            Action(feature='shade', actor=1, recipient=2),  # a1r2
            Action(feature='shade', actor=1, recipient=3),  # a1r3
            Action(feature='shade', actor=2, recipient=1),  # a2r1
            Action(feature='shade', actor=2, recipient=3),  # a2r3
            Action(feature='shade', actor=3, recipient=1),  # a3r1
            Action(feature='shade', actor=3, recipient=2),  # a3r2
            Action(feature='texture', actor=1, recipient=2),  # a1r2
            Action(feature='texture', actor=1, recipient=3),  # a1r3
            Action(feature='texture', actor=2, recipient=1),  # a2r1
            Action(feature='texture', actor=2, recipient=3),  # a2r3
            Action(feature='texture', actor=3, recipient=1),  # a3r1
            Action(feature='texture', actor=3, recipient=2)  # a3r2
        ]

    def next_state_sample(self, s: State, a: Action, rng: Random = random) -> State:
        '''Sample next state based on feature-specific interactions.'''
        actor_shape = getattr(s, f'shape{a.actor}')
        recipient_shape = getattr(s, f'shape{a.recipient}')
        
        # Initialize new shape with current recipient values
        new_sides = recipient_shape.sides
        new_shade = recipient_shape.shade
        new_texture = recipient_shape.texture
        
        # Update only the selected feature
        if a.feature == 'sides':
            if rng.random() < self.SHAPE_TRANSITION_PROB:
                new_sides = actor_shape.sides
            else:
                available_shapes = [shape for shape in self.SHAPE_LIST if shape != actor_shape.sides]
                new_sides = rng.choice(available_shapes)
                
        elif a.feature == 'texture':
            # Texture always cycles when texture feature is selected
            ...
            new_texture = self.TEXTURE_LIST[next_texture_idx]
            
        elif a.feature == 'shade':
            ...
            if actor_shape.shade == recipient_shape.shade:
                if recipient_shape.shade in ['low', 'high']:
                    if rng.random() < 0.8:  # 80% chance to become medium
                        new_shade = 'medium'
                    # else 20% chance to stay the same
                elif recipient_shape.shade == 'medium':
                    if rng.random() < 0.2:  # 20% chance to change
                        new_shade = rng.choice(['low', 'high'])
                    # else 80% chance to stay medium
            else:
                # Move one step towards actor's shade
                if actor_shade_idx > recipient_shade_idx:
                    new_shade = self.SHADE_LIST[recipient_shade_idx + 1]
                else:
                    new_shade = self.SHADE_LIST[recipient_shade_idx - 1]
        
        # Create new shape and state
        new_shape = Shape(sides=new_sides, shade=new_shade, texture=new_texture)
        new_shapes = {
            'shape1': s.shape1,
            'shape2': s.shape2,
            'shape3': s.shape3
        }
        new_shapes[f'shape{a.recipient}'] = new_shape
    ...
    def get_possible_next_states(self, s: State, a: Action) -> Sequence[State]:
        ...
        # Initialize possible values for each feature with current values
        possible_sides = [recipient_shape.sides]
        possible_shades = [recipient_shape.shade]
        possible_textures = [recipient_shape.texture]
        
        # Update possibilities only for the selected feature
        if a.feature == 'sides':
            # Clear the initial list that had the recipient's current sides
            possible_sides = []
            # Add actor's sides (0.8 probability outcome)
            possible_sides.append(actor_shape.sides)
            # Add other possible sides (0.2 probability split between them)
            possible_sides.extend([side for side in self.SHAPE_LIST if side != actor_shape.sides])
            
        elif a.feature == 'texture':
            # If texture is chosen, recipient texture always cycles to next value.
            current_texture_idx = self.TEXTURE_LIST.index(recipient_shape.texture)
            next_texture_idx = (current_texture_idx + 1) % len(self.TEXTURE_LIST)
            ...
            if actor_shape.shade == recipient_shape.shade:
                if recipient_shape.shade in ['low', 'high']:
                    possible_shades = [
                        recipient_shape.shade,  # 20% chance stay same
                        'medium'               # 80% chance become medium
                    ]
                elif recipient_shape.shade == 'medium':
                    possible_shades = [
    ...
    def transition_probability(self, s: State, a: Action, ns: State) -> float:
        ...
        # Initialize probabilities for each feature
        sides_prob = 1.0
        shade_prob = 1.0
        texture_prob = 1.0
        
        # Check only the selected feature's transition probability
        if a.feature == 'sides':
            if next_recipient_shape.sides == actor_shape.sides:
                sides_prob = self.SHAPE_TRANSITION_PROB
            elif next_recipient_shape.sides != actor_shape.sides:
                available_shapes = [shape for shape in self.SHAPE_LIST if shape != actor_shape.sides]
                sides_prob = (1 - self.SHAPE_TRANSITION_PROB) / len(available_shapes)
            # Verify other features didn't change
            if (next_recipient_shape.shade != recipient_shape.shade or 
                next_recipient_shape.texture != recipient_shape.texture):
                raise ValueError("Shade or texture changed when sides feature was selected.")
                return 0.0
            ...
            if next_recipient_shape.texture == self.TEXTURE_LIST[expected_texture_idx]:
                ...
                return 0.0
            # Verify other features didn't change
            if (next_recipient_shape.sides != recipient_shape.sides or 
                next_recipient_shape.shade != recipient_shape.shade):
                raise ValueError("Sides or shade changed when texture feature was selected.")  
                return 0.0
            ...
            if actor_shape.shade == recipient_shape.shade:
                if recipient_shape.shade in ['low', 'high']:
                    if next_recipient_shape.shade == 'medium':
                        shade_prob = 0.8  # 80% chance to become medium
                    elif next_recipient_shape.shade == recipient_shape.shade:
                        shade_prob = 0.2  # 20% chance to stay same
                    else:
                        return 0.0
                elif recipient_shape.shade == 'medium':
                    if next_recipient_shape.shade == 'medium':
                        shade_prob = 0.8  # 80% chance to stay medium
                    elif next_recipient_shape.shade in ['low', 'high']:
                        shade_prob = 0.1  # 10% chance each for low/high
                    else:
                        raise ValueError("Shade changed when shade feature was selected.")
                        return 0.0
            else:
                # Different shade case - move one step towards actor's shade
                recipient_idx = self.SHADE_LIST.index(recipient_shape.shade)
        ...
        # Return the probability for the selected feature
        if a.feature == 'sides':
            return sides_prob
        elif a.feature == 'texture':
            return texture_prob
        elif a.feature == 'shade':  # shade
            return shade_prob
        else:
            raise ValueError("Invalid feature")
    ...
    def plot_state_transitions(self, state: State, action: Action, n_samples: int = 10000):
        ...
        ax1.set_title(f'Transition Probabilities\nFeature: {action.feature}')
        ax1.legend()
        
        # Plot 2: Feature-specific changes
        recipient_idx = action.recipient
        recipient_shape = getattr(state, f'shape{recipient_idx}')
        actor_shape = getattr(state, f'shape{action.actor}')
        
        # Initialize feature-specific tracking
        feature_changes = {}
        
        if action.feature == 'sides':
            # Track specific side changes
            feature_changes = {
                'To Actor Side': 0,  # Changed to actor's side
                'To Other Side': 0,  # Changed to different side
                'No Change': 0       # Stayed the same
            }
            for ns in samples:
                new_recipient = getattr(ns, f'shape{recipient_idx}')
                if new_recipient.sides == actor_shape.sides:
                    feature_changes['To Actor Side'] += 1
                elif new_recipient.sides != recipient_shape.sides:
                    feature_changes['To Other Side'] += 1
                else:
                    feature_changes['No Change'] += 1
                
        elif action.feature == 'texture':
            # Track texture cycling
            next_texture_idx = (self.TEXTURE_LIST.index(recipient_shape.texture) + 1) % len(self.TEXTURE_LIST)
            expected_texture = self.TEXTURE_LIST[next_texture_idx]
            feature_changes = {
                'Cycled': 0,      # Texture cycled as expected
                'Unexpected': 0    # Unexpected change
            }
            for ns in samples:
                new_recipient = getattr(ns, f'shape{recipient_idx}')
                if new_recipient.texture == expected_texture:
                    feature_changes['Cycled'] += 1
                else:
                    feature_changes['Unexpected'] += 1
                
        elif action.feature == 'shade':
            # Track shade changes
            if actor_shape.shade == recipient_shape.shade:
                feature_changes = {
                    'To Medium': 0,
                ...
                for ns in samples:
                    new_recipient = getattr(ns, f'shape{recipient_idx}')
                    if new_recipient.shade == 'medium':
                        feature_changes['To Medium'] += 1
                    elif new_recipient.shade == recipient_shape.shade:
                        feature_changes['Stayed Same'] += 1
                    else:
                        feature_changes['To Low/High'] += 1
            else:
                # Different shade case
                feature_changes = {
                    'Moved Toward Actor': 0,
                    'Other Change': 0,
                    'No Change': 0
                }
                expected_idx = self.SHADE_LIST.index(recipient_shape.shade)
                expected_idx += 1 if self.SHADE_LIST.index(actor_shape.shade) > expected_idx else -1
                expected_shade = self.SHADE_LIST[expected_idx]
                
                for ns in samples:
                    new_recipient = getattr(ns, f'shape{recipient_idx}')
                    if new_recipient.shade == expected_shade:
                        feature_changes['Moved Toward Actor'] += 1
                    elif new_recipient.shade != recipient_shape.shade:
                        feature_changes['Other Change'] += 1
                    else:
                        feature_changes['No Change'] += 1
        
        # Convert to percentages
        total_samples = float(n_samples)
        feature_changes = {k: (v / total_samples) * 100 for k, v in feature_changes.items()}
        
        # Plot feature-specific changes
        ax2.bar(feature_changes.keys(), feature_changes.values())
        ax2.set_ylabel('Percentage (%)')
        ax2.set_title(f'Feature Changes: {action.feature}')
        
        # Add percentage labels on top of bars
        for i, (k, v) in enumerate(feature_changes.items()):
        ...
        print(f"\nSummary of {action.feature} transitions:")
